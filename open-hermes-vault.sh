...
for c in "$HERE/gocryptfs" "$(command -v gocryptfs 2>/dev/null || true)" "$HOME/.local/bin/gocryptfs"; do
    if [ -n "$c" ] && [ -x "$c" ]; then GOCRYPTFS="$c"; break; fi
done
...
[ -f "$VAULT/gocryptfs.conf" ] || { echo "ERROR: no vault found at $VAULT (run ./encrypt-stick.sh first)"; exit 1; }
mkdir -p "$MOUNT"
if mountpoint -q "$MOUNT" 2>/dev/null; then echo "Already open at $MOUNT"; exit 0; fi
